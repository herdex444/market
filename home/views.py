...
def signup(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            newprofile.save()
            login(request, user)
            messages.success(request, f'Congratulations, {{user.first_name}}, Your registration is successful.....')
            return redirect('index')
        else:
            messages.error(request, form.errors)
...
def profile_update(request):
    ...
    if request.method == 'POST':
        ...
        if update.is_valid():
            user = update.save()
            new = user.first_name
            messages.success(request, f'congratulations{{new}}, Your update is successful')
...
def cart(request):
    if request.method =='POST':
        ...
        if cart:
            basket = Cart.objects.filter(product_id=product.id, user__username= request.user.username, paid = False)
            if basket:
...
def pay(request):
    if request.method == 'POST':
        api_key = 'sk_test_70e10c6e5713b7b5eff8fc42d76ff9d62d132232'
        curl = 'https://api.paystack.co/transaction/initialize'
        cburl = 'http://54.175.244.210/callback/'
...
def search(request):
    if request.method == 'POST':
        ...
        searched_items = Product.obejects.filter(searched)
